...
class FFACommand extends Command {
    ...
    public function execute(CommandSender $sender, $alias, array $args) {
        ...
        if(!($sender instanceof Player)) {
            return $sender->sendMessage(TextFormat::RED . "Only in-game!");
        }
        if(!(isset($args[0]))) {
        ...
        if(!(in_array($args[0], $subcommands))) {
            return $sender->sendMessage(TextFormat::RED . "Unknown subcommand");
        }
        if($args[0] == "join") {
            if(!($sender->getLevel() == Server::getInstance()->getDefaultLevel())) {
                return $sender->sendMessage(TextFormat::RED . "You are not in lobby!");
            }
            if(!(isset($args[1]))) {
                return $sender->sendMessage("Usage: /ffa join <soup|nodebuff|gapple>");
            }
            if(!($args[1] == "soup" or $args[1] == "nodebuff" or $args[1] == "gapple")) {
                return $sender->sendMessage(TextFormat::RED . "Mode not found!");
            }
            if($args[1] == "soup") {
                $this->main->joinArena($sender, "soup");
                return $sender->sendMessage(TextFormat::GREEN . "You've joined Soup FFA! Leave: /ffa quit");
            }
            if($args[1] == "nodebuff") {
                $this->main->joinArena($sender, "nodebuff");
                return $sender->sendMessage(TextFormat::GREEN . "You've joined Nodebuff FFA! Leave: /ffa quit");
            }
            if($args[1] == "gapple") {
                $this->main->joinArena($sender, "gapple");
                return $sender->sendMessage(TextFormat::GREEN . "You've joined Gapple FFA! Leave: /ffa quit");
            }
        }
        if($args[0] == "quit") {
            ...
            if(in_array($sender->getLevel(), $arenas)) {
                ...
                $sender->sendMessage(TextFormat::YELLOW . "You've left from arena!");
            } else {
                return $sender->sendMessage(TextFormat::RED . "You are not in FFA!");
            }
        }
        if($args[0] == "kills") {
            $kills = $this->main->getKills($sender);
            return $sender->sendMessage(TextFormat::YELLOW . "Your FFA kills: " . $kills);
        }
        if($args[0] == "info") {
